...
| [design/ir-plan.md](docs/design/ir-plan.md) | Implementing it. Phases 1 to 6 of 9 done. |
...
Semantic resolution has started: the `ir` schema, the declaration table, the interned type table, name resolution, and the spec's recursion rules are in, so a single-package model lowers with its sugar resolved to prelude types and its names bound. The prelude is loaded from TDL source rather than built in, so `[T]` means whatever the loaded prelude says `List` is. `tdl ir` prints the result. Imports resolve across packages without inlining the dependency, mixins expand, and the class satisfaction index answers from declared conformance and ground instances. Constraints, defaults, and targets are still to come, and there are no code-generation backends.
