...
- `internal/ebnf` — the linter for `docs/grammar.ebnf` and `docs/notation.ebnf`: a scanner plus the checks it can make without a full parse, including holding every quoted terminal to `lex` so a spelling the grammar invents is an error rather than a rule that can never match. `Options` says what a file may leave undefined; `GrammarOptions` is the one for `docs/grammar.ebnf`. Private.
...
The grammar is written in Wirth syntax notation, the dialect the Go and Oberon reports use, not ISO 14977: a production ends with `.` rather than `;`, and items in a sequence are juxtaposed rather than separated by `,`. `docs/notation.ebnf` describes that notation in itself. An ISO EBNF tool reads either file as an error from its first production, which is why `internal/ebnf` exists and why VS Code is pointed at `igochkov.vscode-ebnf` for highlighting alone. `TestDocsAreClean` fails the build when either file stops linting clean.
